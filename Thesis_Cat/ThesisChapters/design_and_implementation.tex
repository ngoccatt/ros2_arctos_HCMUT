...
\begin{itemize}
    \item \textbf{Khối thiết bị}: Đây là các thiết bị được điều khiển bởi ROS2, bao gồm cánh tay robot, tay gắp và camera để cung cấp dữ liệu ngoại quan:
    ...
    \item \textbf{Tay gắp Gripper-A}
    
    Phần tay gắp được nối ở đầu của cánh tay robot, có nhiệm vụ gắp vật thể và được điều khiển trực tiếp bởi ROS2.
    ...
    \end{itemize}
    \item \textbf{Khối điều khiển: PC Controller}: 
    ...
    \item \textbf{Khối ứng dụng VR: Meta Quest 3}

    Meta Quest 3 được nhóm lựa chọn là thiết bị VR được sử dụng trong đồ án. Một ứng dụng VR sẽ được phát triển tận dụng Unity Engine và thư viện VR được cung cấp và phát triển bởi Meta, với mục tiêu xây dựng ứng dụng điều khiển, tương tác hiệu quả, trực quan, đồng bộ tốt với cánh tay thực tế. Việc cho phép người dùng sử dụng VR để điều khiển cánh tay sẽ đem lại trải nghiệm sinh động, trực quan, tăng hiệu quả điều khiển cho người dùng từ xa mà không đánh đổi sự tiện lợi, cung cấp đầy đủ các tính năng điều khiển thông dụng như trên thiết bị truyền thống.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/Implementation/RC5_architecture.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/Implementation/ros2_architecture.jpg}
...
\centering
\begin{tikzpicture}[
    drvN/.style={draw=blue!60, fill=blue!12, rounded corners=3pt,
                 minimum width=3.2cm, minimum height=0.7cm, text centered, font=\small},
    hwN/.style={draw=red!50, fill=red!10, rounded corners=3pt,
                minimum width=3.2cm, minimum height=0.7cm, text centered, font=\small},
    grpB/.style={draw=blue!40, dashed, rounded corners=5pt, inner sep=10pt, fill=blue!3},
    myA/.style={-Stealth, thick}
]
% Arm Driver column (software driver nodes only)
\node[drvN] (md) at (0,1.5)    {\texttt{MotorDriver}};
\node[drvN] (u1) at (0,0.5)    {\texttt{UartProtocol}};
% Hand Driver column (software driver nodes only)
\node[drvN] (sd) at (4.5,1.5)  {\texttt{ServoDriver}};
\node[drvN] (u2) at (4.5,0.5)  {\texttt{UartProtocol}};
% Hardware devices -- outside the driver software groups
\node[hwN]  (r5) at (0,-0.7)   {\textbf{RC5}};
\node[hwN]  (ha) at (4.5,-0.7) {\textbf{Driver-Hat-A}};
\draw[myA] (md) -- (u1);
\draw[myA] (u1) -- (r5);
\draw[myA] (sd) -- (u2);
\draw[myA] (u2) -- (ha);
\begin{scope}[on background layer]
  \node[grpB, fit=(md)(u1), label=above:{\small\textbf{Arm Driver}}] {};
  \node[grpB, fit=(sd)(u2), label=above:{\small\textbf{Hand Driver}}] {};
\end{scope}
\end{tikzpicture}
\caption{Sơ đồ khối Driver}
\label{fig:driver-block}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Images/Implementation/solidwork.jpg}
    \caption{Mô hình robot trong SolidWorks đã tích hợp tay gắp. Plugins sẽ được sử dụng để export chi tiết cánh tay ra file URDF để tham khảo và hiện thực}
    \label{fig:solidwork_example}
\end{figure}
...
\subsection{Khối Hardware Interface}
...
\centering
\begin{tikzpicture}[
    hiN/.style={draw=green!60, fill=green!12, rounded corners=3pt,
                minimum width=3.5cm, minimum height=0.7cm, text centered, font=\small},
    layN/.style={draw=gray!50, fill=gray!10, rounded corners=3pt,
                 minimum width=9cm, minimum height=0.7cm, text centered, font=\footnotesize},
    grpB/.style={draw=green!50, dashed, rounded corners=5pt, inner sep=8pt, fill=green!3},
    BD/.style={Stealth-Stealth, thick, gray!70}
]
\node[layN] (cl) at (0,2)      {\textbf{Controllers}};
\node[hiN]  (di) at (-2.2,0.8) {\texttt{DensoInterface}};
\node[hiN]  (dh) at ( 2.2,0.8) {\texttt{DensoHandInterface}};
\draw[BD] (cl.south -| di.north) -- (di.north) node[midway, left,  font=\tiny]{cmd/state};
\draw[BD] (cl.south -| dh.north) -- (dh.north) node[midway, right, font=\tiny]{cmd/state};
\begin{scope}[on background layer]
  \node[grpB, fit=(di)(dh), label=above:{\small\textbf{Hardware Interface}}] {};
\end{scope}
\end{tikzpicture}
\caption{Sơ đồ khối Hardware Interface trong kiến trúc Ros2-Control}
\label{fig:hw-interface-block}
\end{figure}
...
Ví dụ về cấu hình cho 1 trục của \lstinline[style=inlinecode]{denso_hardware_interface} và DensoHandInterface như bên dưới:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/Implementation/write_arm.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\linewidth]{Images/Implementation/write_hand.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/Implementation/read_arm.jpg}
    \caption{Luồng đọc (Read) của DensoInterface}
    \label{fig:placeholder}
\end{figure}

Trong luồng đọc của DensoInterface, dữ liệu thực tế được phản hồi từ cánh tay cần được xử lý và đọc vào các state interfaces. Ở đầu hàm \lstinline[style=inlinecode]{read}, \lstinline[style=inlinecode]{processUartMessage()} của Motor Driver được gọi. Trong \lstinline[style=inlinecode]{processUartMessage}, hàm sẽ lần lượt lấy từng dữ liệu UART nhận được từ buffer của module UART và thực hiện decode (giải mã) nó theo format dữ liệu đã được đề xuất ở phần trước, sử dụng hàm decode được hỗ trợ từ module Uart. Sau khi decode, ta có được giá trị góc xoay của cả 6 trục trong một mảng số thực. Các giá trị góc xoay này sẽ lần lượt được lưu vào \lstinline[style=inlinecode]{JointConfig} của từng trục, sẵn sàng để được truy vấn.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\linewidth]{Images/Implementation/read_hand.jpg}
...
\centering
\begin{tikzpicture}[
    cN/.style={draw=gray!60, fill=gray!10, rounded corners=3pt,
               minimum width=4cm, minimum height=0.9cm,
               text width=3.8cm, text centered, align=center, font=\small},
    mN/.style={draw=gray!70, fill=gray!20, rounded corners=3pt,
               minimum width=14cm, minimum height=0.7cm,
               text centered, font=\small},
    grpB/.style={draw=gray!60, dashed, rounded corners=5pt, inner sep=8pt},
    myA/.style={-Stealth, thick}
]
\node[mN] (mgr) at (0,2)    {\textbf{Controller Manager}};
\node[cN] (jtc) at (-5,0.6) {\texttt{denso\_arm\_controller}\\{\tiny JointTrajectoryController}};
\node[cN] (gac) at ( 0,0.6) {\texttt{denso\_hand\_controller}\\{\tiny GripperActionController}};
\node[cN] (jsb) at ( 5,0.6) {\texttt{joint\_state\_broadcaster}\\{\tiny JointStateBroadcaster}};
\draw[myA] (mgr.south -| jtc.north) -- (jtc.north);
\draw[myA] (mgr.south -| gac.north) -- (gac.north);
\draw[myA] (mgr.south -| jsb.north) -- (jsb.north);
\begin{scope}[on background layer]
  \node[grpB, fit=(mgr)(jtc)(gac)(jsb)] {};
\end{scope}
\end{tikzpicture}
\caption{Sơ đồ khối Controllers trong kiến trúc Ros2-Control}
\label{fig:controllers-block}
\end{figure}
...
\centering
\begin{tikzpicture}[
    mvN/.style={draw=yellow!70!black, fill=yellow!15, rounded corners=3pt,
                minimum width=2.8cm, minimum height=0.75cm, text centered, font=\small},
    appN/.style={draw=orange!70, fill=orange!12, rounded corners=3pt,
                 minimum width=3.3cm, minimum height=0.9cm,
                 text centered, align=center, font=\small},
    grpB/.style={draw=yellow!60!black, dashed, rounded corners=5pt, inner sep=8pt, fill=yellow!5},
    appG/.style={draw=orange!50, dashed, rounded corners=5pt, inner sep=8pt, fill=orange!5},
    myA/.style={-Stealth, thick}
]
\node[appN] (dr) at (-3,2.3) {\texttt{Denso Remote Control}\\{\tiny Action Server}};
\node[appN] (ds) at ( 1.5,2.3) {\texttt{Denso MoveIt Servo}\\{\tiny Service Provider}};
\node[mvN] (sc) at (-5.25,0.5) {SimpleCtrl Manager};
\node[mvN] (pl) at (-1.75,0.5) {Path Planning};
\node[mvN] (sv) at ( 1.75,0.5) {Moveit Servo};
\node[mvN] (rv) at ( 5.25,0.5) {\textbf{Rviz}};
\draw[myA] (dr.south) -- ++(0,-0.3) -| (pl.north);
\draw[myA] (ds.south) -- (sv.north);
\begin{scope}[on background layer]
  \node[grpB, fit=(pl)(sc)(sv)(rv), label=below:{\small\textbf{MoveIt2}}] {};
  \node[appG, fit=(dr)(ds), label=above:{\small\textbf{Lớp ứng dụng}}] {};
\end{scope}
\end{tikzpicture}
\caption{Sơ đồ khối MoveIt2 và lớp ứng dụng}
\label{fig:moveit2-block}
\end{figure}
...
\centering
\begin{tikzpicture}[
    rbN/.style={draw=purple!60, fill=purple!10, rounded corners=3pt,
                minimum width=8cm, minimum height=0.85cm,
                text centered, font=\small},
    appN/.style={draw=orange!70, fill=orange!15, rounded corners=3pt,
                 minimum width=3.8cm, minimum height=1cm,
                 text centered, align=center, font=\small},
    mvN/.style={draw=yellow!70!black, fill=yellow!15, rounded corners=3pt,
                minimum width=8cm, minimum height=0.85cm,
                text centered, font=\small},
    appG/.style={draw=orange!60, dashed, rounded corners=5pt, inner sep=10pt, fill=orange!5},
    myA/.style={-Stealth, thick},
    myAB/.style={Stealth-Stealth, thick, gray!70}
]
\node[rbN]  (rb) at (0, 3)     {\textbf{RosBridge Server}};
\node[appN] (dr) at (-2.5, 1.5) {\texttt{Denso Remote Control}\\{\tiny Action Server}};
\node[appN] (ds) at ( 2.5, 1.5) {\texttt{Denso MoveIt Servo}\\{\tiny Service Provider}};
\node[mvN]  (mv) at (0, 0)     {\textbf{MoveIt2} (Path Planning / MoveIt Servo)};
\draw[myAB] (rb.south -| dr.north) -- (dr.north) node[midway, left,  font=\tiny]{Goal / Result};
\draw[myAB] (rb.south -| ds.north) -- (ds.north) node[midway, right, font=\tiny]{Req / Resp};
\draw[myA] (dr.south) -- (mv.north -| dr.south) node[midway, left,  font=\tiny]{Plan \& Execute};
\draw[myA] (ds.south) -- (mv.north -| ds.south) node[midway, right, font=\tiny]{Servo Cmd};
\begin{scope}[on background layer]
  \node[appG, fit=(dr)(ds), label=above:{\small\textbf{Lớp ứng dụng}}] {};
\end{scope}
\end{tikzpicture}
\caption{Sơ đồ khối lớp ứng dụng}
\label{fig:app-block}
\end{figure}
...
\begin{tikzpicture}[
  node distance=0.6cm,
  startstop/.style={rectangle, rounded corners=5pt, draw=teal!80!black, fill=teal!15,
    text width=6.5cm, align=center, minimum height=0.7cm, font=\small\bfseries},
  process/.style={rectangle, draw=black!50, fill=blue!7, rounded corners=2pt,
    text width=6.5cm, align=center, minimum height=0.7cm, font=\small},
  decision/.style={diamond, draw=black!60, fill=orange!15, aspect=3,
    text width=3.4cm, align=center, font=\footnotesize, inner sep=2pt},
  fail/.style={rectangle, draw=red!60, fill=red!8, rounded corners=3pt,
    text width=4cm, align=center, minimum height=0.6cm, font=\footnotesize},
  arrow/.style={->, >=Stealth, semithick},
  earrow/.style={->, >=Stealth, semithick, red!70},
]

\node[startstop] (recv)
  {Action Client gửi \texttt{MoveToPose} goal};

\node[decision, below=0.5cm of recv] (valid)
  {Goal hợp lệ?};

\node[fail, right=2.6cm of valid] (reject)
  {Trả về \texttt{REJECT}\\
   (\texttt{joints.size() < 6})};

\node[process, below=0.7cm of valid] (accept)
  {\texttt{ACCEPT\_AND\_EXECUTE}\\
   Spawn \texttt{execute()} trên thread riêng};

\node[decision, below=0.5cm of accept] (cancel)
  {Yêu cầu hủy goal?};

\node[fail, right=2.6cm of cancel] (canceled)
  {\texttt{goal\_handle->canceled(result)}};

\node[process, below=0.7cm of cancel] (planning)
  {Thiết lập target:\\
   \textit{Nếu} \texttt{use\_pose}: \texttt{setPoseTarget(pose)}\\
   \textit{Nếu không}: \texttt{setJointValueTarget(joints)}\\
   Feedback: \texttt{step = "planning"}};

\node[decision, below=0.6cm of planning] (planok)
  {\texttt{plan()} thành công?};

\node[fail, right=2.6cm of planok] (planfail)
  {Feedback:\\
   \texttt{step = "planning\_failed:..."}\\[2pt]
   \texttt{abort(result)}};

\node[process, below=0.7cm of planok] (executing)
  {Feedback: \texttt{step = "executing"}\\
   \texttt{move\_group->execute(my\_plan)}};

\node[decision, below=0.6cm of executing] (execok)
  {\texttt{execute()} thành công?};

\node[fail, right=2.6cm of execok] (execfail)
  {Feedback:\\
   \texttt{step = "execute\_failed:..."}\\[2pt]
   \texttt{abort(result)}};

\node[startstop, below=0.7cm of execok] (done)
  {Feedback: \texttt{step = "completed"}\\
   \texttt{result->completed = true}\\
   \texttt{goal\_handle->succeed(result)}};

% Main flow
\draw[arrow] (recv)      -- (valid);
\draw[arrow] (valid)     -- node[left,  font=\footnotesize]{Có}    (accept);
\draw[arrow] (accept)    -- (cancel);
\draw[arrow] (cancel)    -- node[left,  font=\footnotesize]{Không} (planning);
\draw[arrow] (planning)  -- (planok);
\draw[arrow] (planok)    -- node[left,  font=\footnotesize]{Có}    (executing);
\draw[arrow] (executing) -- (execok);
\draw[arrow] (execok)    -- node[left,  font=\footnotesize]{Có}    (done);

% Error exits
\draw[earrow] (valid)   -- node[above, font=\footnotesize]{Không} (reject);
\draw[earrow] (cancel)  -- node[above, font=\footnotesize]{Có}    (canceled);
\draw[earrow] (planok)  -- node[above, font=\footnotesize]{Không} (planfail);
\draw[earrow] (execok)  -- node[above, font=\footnotesize]{Không} (execfail);

\end{tikzpicture}
\caption{Luồng hoạt động của \texttt{RemoteControlServer} (\texttt{remote\_control\_server.cpp})}
\label{fig:remote-control-flow}
\end{figure}
...
\begin{tikzpicture}[
    cN/.style={draw=gray!60, fill=gray!10, rounded corners=3pt,
               minimum width=9cm, minimum height=0.7cm, text centered, font=\small},
    gN/.style={draw=orange!60, fill=orange!12, rounded corners=3pt,
               minimum width=9cm, minimum height=0.85cm,
               text centered, align=center, font=\small},
    sN/.style={draw=orange!80, fill=orange!25, rounded corners=3pt,
               minimum width=9cm, minimum height=0.85cm,
               text centered, align=center, font=\small},
    grpB/.style={draw=orange!50, dashed, rounded corners=5pt, inner sep=8pt, fill=orange!5},
    myA/.style={-Stealth, thick}
]
\node[cN] (ct) at (0,2)    {\textbf{Controllers} (JointTrajectoryController, GripperActionController)};
\node[gN] (gz) at (0,0.8)  {\texttt{GazeboSimSystem}\\{\footnotesize thay thế Hardware Interface và Driver}};
\node[sN] (sm) at (0,-0.5) {\textbf{Gazebo Simulation}\\{\footnotesize Mô phỏng cánh tay VS-6577 và tay gắp Gripper-A}};
\draw[myA] (ct) -- (gz) node[midway, right, font=\small]{cmd/state};
\draw[myA] (gz) -- (sm);
\begin{scope}[on background layer]
  \node[grpB, fit=(gz)(sm), label=below:{\small\textbf{Gazebo}}] {};
\end{scope}
\end{tikzpicture}
\caption{Sơ đồ mô phỏng với Gazebo thay thế phần Hardware Interface và Driver}
\label{fig:gazebo-block}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Thêm thuộc tính friction (ma sát), damping (giảm chấn) vào cho các trục trong file URDF}: Nếu thiếu hai thông số friction và damping, các trục gần như sẽ bị vọt lố (overshoot) do quán tính, khiến nó xoay xa hơn so với vị trí yêu cầu. Bộ số friction, damping hợp lý sẽ giữ cho cánh tay có được độ ổn định, có thể giữ vị trí góc được điều khiển và xoay với tốc độ gần tương đồng với thiết bị thật. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Images/Implementation/rosbridge_architecture.jpg}
...
Source code của hệ thống điều khiển bằng ROS2 có thể tìm ở đây: \href{https://github.com/ngoccatt/ros2_arctos_HCMUT}{ros2\_arctos\_HCMUT}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Implementation/transferfromros.jpg}
        \caption{Thiết lập Transfer URDF from ROS}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.68\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Implementation/transferfromros_done.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Images/Implementation/vr_architecture.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Images/Implementation/camerarig.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Images/Implementation/vr_ros_architecture.jpg}
    \caption{Kiến trúc tổng quan ứng dụng VR: Module giao tiếp ROS2}
    \label{fig:vr-app-architecture}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/Implementation/connect_panel.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{Images/Implementation/image_subscriber.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{Images/Implementation/jointstatesubscriber.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{Images/Implementation/moveit_servo_panel.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/Implementation/generate_action.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/Implementation/unitygrippercommand.jpg}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Implementation/jointrotate.jpg}
        \caption{Joint Rotate Panel}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.68\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Implementation/futuredenso_articulation.jpg}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.31\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Implementation/robotgrabrotate.jpg}
        \caption{Robot Grab Rotate}
    \end{subfigure}
    \begin{subfigure}{0.65\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Implementation/robotgrabrotate_transformer.jpg}
...
Source code cho phần hiện thực ứng dụng VR có thể tìm thấy ở đây: \href{https://github.com/ngoccatt/denso_arm_metaquest}{denso\_arm\_metaquest}
