"""
Visual Inspection Inference on CPU (Keras FP32)
Model: best_model.h5
Supports webcam and dataset evaluation with binary-specific quality metrics.

Dataset mode:
    python inference_testing_cpu.py --mode dataset --dataset dataset/split/test \
        --output results/cpu_results.json --charts results/cpu_charts/
...
class InspectionCPUInference:
    ...
    def infer(self, frame):
        inp = np.expand_dims(self.preprocess(frame), axis=0)
...
def run_webcam_demo(model_path, metadata_path=None, camera_id=0, threshold=0.7):
    inferencer = InspectionCPUInference(model_path, metadata_path)
    ...
    while True:
        ...
        cv2.imshow("Visual Inspection - CPU (FP32)", frame)
...
def run_dataset_inference(model_path, metadata_path, dataset_path, output_file=None, charts_dir=None):
    ...
    results = []
    y_true, y_pred = [], []
    y_score_fail = []   # FAIL-class probability for each sample (for ROC)
    class_accuracy = {}
    total_latency = 0
    image_count = 0
    ...
    print(f"Running CPU inference on dataset: {dataset_path}")
    print("=" * 60)
    ...
    # Binary quality metrics — FAIL is the positive (defect) class
    y_true_bin = [1 if t == fail_label else 0 for t in y_true]
    y_pred_bin = [1 if p == fail_label else p for p in y_pred]
    ...
    if charts_dir:
        ...
        # 1. Confusion matrix (label-based 2x2)
        ...
        plt.title("Confusion Matrix - CPU FP32", fontsize=13, fontweight="bold")
        ...
        plt.ylim(0, 108)
        plt.ylabel("Accuracy (%)")
        plt.title("Per-Class Accuracy - CPU FP32", fontsize=13, fontweight="bold")
        plt.legend()
        plt.tight_layout()
        ...
        # 3. Confidence score histogram (FAIL-class score, separated by ground truth)
        ...
        plt.ylabel("Density")
        plt.title("FAIL Score Distribution by Ground Truth - CPU FP32", fontweight="bold")
        plt.legend()
        plt.tight_layout()
        ...
        if not np.isnan(auc):
            ...
            plt.title("ROC Curve - CPU FP32", fontweight="bold")
            plt.legend()
            plt.tight_layout()
        ...
        # 5. Threshold sweep (FNR and FPR vs threshold)
        ...
        ax1.axvline(0.5, color="gray", linestyle=":", lw=1.2, label="Default (0.5)")
        plt.title("Threshold Analysis - CPU FP32", fontweight="bold")
        plt.tight_layout()
        ...
        with open(summary_path, "w") as f:
            f.write("INFERENCE SUMMARY - CPU FP32 MODEL\n")
            f.write("=" * 60 + "\n\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visual Inspection - CPU FP32 Evaluation")
    parser.add_argument("--model", default="models/best_model.h5", help="Keras .h5 model path")
    ...
    if args.mode == "webcam":
        ...
        print("=" * 60)
        print("MODE: Dataset Evaluation (CPU - FP32)")
        print("=" * 60)
