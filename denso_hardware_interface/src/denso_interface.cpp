#include "denso_hardware_interface/denso_interface.hpp"
...
using denso_motor_driver::MotorMode;
...
namespace denso_interface
{
    ...
    DensoInterface::DensoInterface()
        : SystemInterface(),
          node_(std::make_shared<rclcpp::Node>("denso_hardware_interface"))
    {
        uart_protocol_ = std::make_shared<denso_motor_driver::UartProtocol>();
        motor_driver_ = std::make_shared<denso_motor_driver::MotorDriver>(node_);
        motor_driver_->setProtocol(uart_protocol_);
    }

    DensoInterface::~DensoInterface() = default;
    ...
    /* HardwareInfo info could be collected under <ros2_control> tag, and the joints are <joint>. We then seek each joint params via denso_hardware_interface/ros__parameters */
    CallbackReturn DensoInterface::on_init(const hardware_interface::HardwareInfo &info)
    {
    ...
    CallbackReturn DensoInterface::on_configure(const rclcpp_lifecycle::State &previous_state)
    {
    ...
    CallbackReturn DensoInterface::on_activate(const rclcpp_lifecycle::State &previous_state)
    {
    ...
    CallbackReturn DensoInterface::on_deactivate(const rclcpp_lifecycle::State &previous_state)
    {
    ...
    std::vector<hardware_interface::StateInterface> DensoInterface::export_state_interfaces()
    {
    ...
    std::vector<hardware_interface::CommandInterface> DensoInterface::export_command_interfaces()
    {
    ...
    */
    return_type DensoInterface::read(const rclcpp::Time &time, const rclcpp::Duration & /*period*/)
    {
    ...
    */
    return_type DensoInterface::write(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
    {
    ...
    void DensoInterface::initializeMotors()
    {
        for (size_t i = 0; i < info_.joints.size(); i++)
        {
            const auto &joint = info_.joints[i];
    ...
    bool DensoInterface::setupMotorParameters(
...
PLUGINLIB_EXPORT_CLASS(
    denso_interface::DensoInterface,
